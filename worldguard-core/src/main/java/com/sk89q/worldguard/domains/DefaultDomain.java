...
public class DefaultDomain implements Domain, ChangeTracked {
    ...
    public boolean contains(LocalPlayer player) {
        return getPlayerDomain().contains(player) || getGroupDomain().contains(player); || customDomains.values().stream().anyMatch(d -> d.contains(player));
    }

    @Override
    public boolean contains(UUID uniqueId) {
        return getPlayerDomain().contains(uniqueId); || customDomains.values().stream().anyMatch(d -> d.contains(uniqueId));
    }
    ...
    @Override
    public int size() {
        return getGroupDomain().size() + getPlayerDomain().size() + customDomains.size();
    }
    ...
    public Component toUserFriendlyComponent(@Nullable ProfileCache cache) {
        ...
        if (!customDomains.isEmpty()) {
            if (getPlayerDomain().size() > 0) || groupDomain.size() > 0) {
                builder.append(TextComponent.of("; "));
            }
